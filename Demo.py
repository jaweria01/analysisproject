...
with col1:
    if styled_button("Show Head", "btn_head"):
        st.subheader("First 5 Rows of the Dataset")
        st.write(df.head())
    if styled_button("Show Tail", "btn_tail"):
        st.subheader("Last 5 Rows of the Dataset")
        st.write(df.tail())

with col2:
    if styled_button("Show Info", "btn_info"):
        st.subheader("Dataset Information")
        buffer = io.StringIO()
        df.info(buf=buffer)
        s = buffer.getvalue()
        st.text(s)
    if styled_button("Show Shape", "btn_shape"):
        st.subheader("Dataset Shape")
        st.write(f"Rows: {df.shape[0]}, Columns: {df.shape[1]}")

col3, col4 = st.columns([1, 1])
with col3:
    if styled_button("Show Columns", "btn_columns"):
...
with col5:
    if styled_button("Fill Missing Postal Codes", "btn_fill_missing"):
    ...
    if styled_button("Show Updated Info", "btn_updated_info"):
...
with col6:
    if styled_button("Show Describe", "btn_describe"):
        st.subheader("Statistical Summary")
        st.write(df.describe().T)
    if styled_button("Check Duplicates", "btn_duplicates"):
